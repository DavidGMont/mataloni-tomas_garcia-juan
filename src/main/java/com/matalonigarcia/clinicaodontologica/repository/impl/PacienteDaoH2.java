...
public class PacienteDaoH2 implements IDao<Paciente> {
    ...
    public Paciente actualizar(Paciente paciente) {
        ...
        try {
            ...
            PreparedStatement preparedStatement = connection.prepareStatement("UPDATE PACIENTES SET NOMBRE = ?, APELLIDO = ?, DNI = ?, FECHA = ?, DOMICILIO_ID = ? WHERE ID = ?");
            preparedStatement.setString(1, paciente.getNombre());
            preparedStatement.setString(2, paciente.getApellido());
            preparedStatement.setString(3, paciente.getDni());
            preparedStatement.setDate(4, Date.valueOf(paciente.getFechaIngreso()));
            ...
            connection.commit();
            LOGGER.warn("🛑 El paciente con ID {} ha sido actualizado: {}", paciente.getId(), paciente);
        } catch (Exception e) {
            LOGGER.error("💥 Te encontraste con un gran error: {}", e.getMessage());
            e.printStackTrace();
            if (connection != null) {
